...
const incidentTemplates = [
  ...
  { type: 'flood',    severity: 2, description: 'Community isolated, need food and water',                 disasterType: 'flood'      },
  { type: 'trapped',  severity: 4, description: 'Child trapped in debris after wall collapse',             disasterType: 'earthquake' },
  { type: 'medical',  severity: 3, description: 'Outbreak at shelter, need PPE and medical staff',         disasterType: 'other'      },
];

// 20-item resource catalog with tags for matching
const resourceCatalog = [
  { type: 'First Aid Kits',                  tags: ['medical', 'trapped']                          },
  { type: 'Trauma Kits',                     tags: ['medical', 'fire', 'trapped']                  },
  { type: 'Potable Water',                   tags: ['medical', 'flood', 'shelter']                 },
  { type: 'Non-Perishable Food Packs',       tags: ['flood', 'shelter', 'trapped']                 },
  { type: 'Blankets',                        tags: ['shelter', 'flood', 'trapped']                 },
  { type: 'Flashlights',                     tags: ['fire', 'flood', 'trapped', 'medical']         },
  { type: 'Batteries',                       tags: ['fire', 'flood', 'communications']             },
  { type: 'Portable Power Banks',            tags: ['communications', 'medical', 'trapped']        },
  { type: 'Handheld Two-Way Radios',         tags: ['fire', 'flood', 'trapped', 'communications'] },
  { type: 'Reflective Safety Vests',         tags: ['rescue', 'fire', 'flood']                    },
  { type: 'Heavy-Duty Work Gloves',          tags: ['rescue', 'fire', 'trapped']                  },
  { type: 'Multi-Tools / Swiss Army Knives', tags: ['rescue', 'fire', 'trapped']                  },
  { type: 'Tarpaulins',                      tags: ['shelter', 'flood', 'fire']                   },
  { type: 'Ropes',                           tags: ['rescue', 'trapped', 'flood']                 },
  { type: 'Dust Masks / Respirators',        tags: ['fire', 'medical', 'hazmat']                  },
  { type: 'Hand Sanitizer',                  tags: ['medical', 'shelter']                         },
  { type: 'Portable Gas Stoves',             tags: ['shelter', 'flood']                           },
  { type: 'Sleeping Bags',                   tags: ['shelter', 'flood', 'trapped']                },
  { type: 'Fire Extinguishers',              tags: ['fire']                                       },
  { type: 'Water Purification Tablets',      tags: ['flood', 'medical', 'water']                  },
];

const orgResourceTypes = [
  { type: 'water',       unit: 'liters',   baseQty: 5000 },
  { type: 'food',        unit: 'packages', baseQty: 2000 },
  { type: 'medical',     unit: 'kits',     baseQty: 500  },
  { type: 'shelter',     unit: 'spaces',   baseQty: 200  },
...
// ============================================================
// Users — clean usernames, no location suffixes
// ============================================================
...
const volunteers = [
  ...
  { username: 'vol_davis',     password: 'VolPass456!', skills: ['Multilingual', 'Spanish', 'French']                },
  { username: 'vol_taylor',    password: 'VolPass567!', skills: ['Mental Health', 'Crisis Counseling']               },
  { username: 'vol_campbell',  password: 'VolPass678!', skills: ['First Aid/CPR', 'Search & Rescue']                 },
  { username: 'vol_grant',     password: 'VolPass789!', skills: ['Logistics', 'Transport', 'Heavy Equipment Operation'] },
  { username: 'vol_morgan',    password: 'VolPass890!', skills: ['Basic Life Support', 'Triage']                     },
  { username: 'vol_stewart',   password: 'VolPass901!', skills: ['Firefighting', 'Search & Rescue']                  },
  { username: 'vol_reid',      password: 'VolPass012!', skills: ['Swift Water Rescue', 'Flood Response']             },
];
...
/**
 * Picks 3–8 unique items from the 20-item resourceCatalog.
 * Each item gets a random quantity (1–15) and a status.
 */
function buildVolunteerResources() {
  return pickSome(resourceCatalog, 3, 8).map((item) => ({
    type:     item.type,
    quantity: 1 + Math.floor(Math.random() * 15),
    status:   Math.random() > 0.35 ? 'available' : 'deployed',
  }));
}
...
async function seedDatabase() {
  ...
  try {
    ...
    for (const table of [
      'dispatch_decisions', 'resource_requests', 'generated_reports',
      'layer_presets', 'map_layers',
      'sms_incidents', 'media', 'device_tokens', 'messages',
      'preparedness_content', 'volunteer_stats', 'user_alerts',
      'broadcast_alerts', 'resource_allocations', 'notifications',
      'incident_history', 'processed_requests', 'refresh_tokens', 'otps',
    ...
    for (const v of victims) {
      ...
      const pt = randomPointNear(p);
      const { rows } = await client.query(
        `INSERT INTO users (phone, role, location, last_lat, last_lon, last_location_at, created_at)
         VALUES ($1, 'victim', ST_GeogFromText($2), $3, $4, NOW(), NOW() - INTERVAL '${Math.floor(Math.random() * 30)} days')
         RETURNING id`,
        [v.phone, point(pt.lon, pt.lat), pt.lat, pt.lon]
      );
      victimIds.push(rows[0].id);
    }
    console.log(`   ${victimIds.length} victims`);
    ...
    // ── 2. Volunteers ───────────────────────────────────────
    // Spread across all 14 parishes for realistic dispatch distances
    console.log('Seeding volunteers...');
    const volunteerIds = [];
    for (let i = 0; i < volunteers.length; i++) {
      const v  = volunteers[i];
      const p  = parishes[i % parishes.length]; // cycle through parishes
      const pt = randomPointNear(p);
      const resources = buildVolunteerResources();

      const { rows } = await client.query(
        `INSERT INTO users (
           username, password_hash, phone, role, skills, resources,
           active_location_name, active_location_lat, active_location_lon,
           last_lat, last_lon, last_location_at, location,
    ...
    console.log(`   ${volunteerIds.length} volunteers (spread across parishes, 3–8 resources each)`);
    ...
    for (const c of coordinators) {
      ...
      const { rows } = await client.query(
        `INSERT INTO users (
           username, password_hash, phone, role, skills,
           last_lat, last_lon, last_location_at, location, created_at
         )
         VALUES ($1, $2, $3, 'coordinator', $4::jsonb,
                 $5, $6, NOW(),
                 ST_GeogFromText($7),
    ...
    console.log('Seeding incidents...');
    const allResponders = [...volunteerIds, ...responderIds];
    const statuses      = ['active', 'active', 'active', 'in-progress', 'resolved'];
    const incidentIds   = [];
    ...
    for (const t of incidentTemplates) {
      ...
      const { rows } = await client.query(
        `INSERT INTO incidents
           (type, location, severity, description, disaster_type, area_id,
            status, submitted_by, assigned_to, people_affected, resource_needs,
    ...
    // ── 6. Org-level resources ──────────────────────────────
    console.log('Seeding organisation resources...');
    const resourceIds = [];
    for (const rt of orgResourceTypes) {
      const count = 2 + Math.floor(Math.random() * 3);
      for (let i = 0; i < count; i++) {
        const p   = pick(parishes);
        const pt  = randomPointNear(p);
        ...
        const { rows } = await client.query(
          `INSERT INTO resources (type, quantity, unit, location, organisation_id, status, last_updated)
           VALUES ($1, $2, $3, ST_GeogFromText($4), $5, 'available', NOW() - INTERVAL '${Math.floor(Math.random() * 5)} days')
           RETURNING id`,
          [rt.type, qty, rt.unit, point(pt.lon, pt.lat), pick(coordinatorIds)]
        );
        resourceIds.push(rows[0].id);
    ...
    console.log(`   ${resourceIds.length} organisation resources`);
    ...
    for (let i = 0; i < 10; i++) {
      await client.query(
        `INSERT INTO resource_allocations (resource_id, incident_id, quantity, allocated_at)
         VALUES ($1, $2, $3, NOW() - INTERVAL '${Math.floor(Math.random() * 3)} days')`,
        [pick(resourceIds), pick(incidentIds), 5 + Math.floor(Math.random() * 50)]
      );
    }
    console.log('   10 resource allocations');
    ...
    console.log('Seeding notifications...');
    const notifTypes = [
      { type: 'INCIDENT_ASSIGNED', make: (id) => ({ message: `You have been assigned to incident #${id}`,              incidentId: String(id) }) },
      { type: 'STATUS_UPDATE',     make: (id) => ({ message: `Incident #${id} status changed to in-progress`,          incidentId: String(id) }) },
      { type: 'BROADCAST_ALERT',   make: ()   => ({ message: 'Hurricane warning issued for your area. Seek shelter.' })                          },
      { type: 'TASK_COMPLETED',    make: (id) => ({ message: `Incident #${id} has been resolved. Thank you!`,          incidentId: String(id) }) },
      { type: 'NEW_MESSAGE',       make: (id) => ({ message: `New message on incident #${id}`,                         incidentId: String(id) }) },
    ];
    const allUserIds = [...victimIds, ...volunteerIds, ...responderIds, ...coordinatorIds];
    let notifCount = 0;
    for (let i = 0; i < 30; i++) {
      ...
      await client.query(
        `INSERT INTO notifications (user_id, type, data, read, created_at)
         VALUES ($1, $2, $3, $4, NOW() - INTERVAL '${Math.floor(Math.random() * 7)} days ${Math.floor(Math.random() * 24)} hours')`,
        [pick(allUserIds), n.type, JSON.stringify(n.make(pick(incidentIds))), Math.random() > 0.5]
      );
      notifCount++;
    }
    console.log(`   ${notifCount} notifications`);
    ...
    console.log('Seeding broadcast alerts...');
    const broadcastMessages = [
      { message: '⚠️ HURRICANE WARNING: Category 3 hurricane approaching southern coast. Coastal residents should evacuate immediately.',       roles: 'all',                 hours: 2  },
      { message: '🏥 MEDICAL SUPPLIES: Emergency supplies at Kingston General. Volunteers needed for distribution.',                            roles: 'volunteer,responder', hours: 12 },
      { message: '🚧 ROAD CLOSURE: A3 highway blocked between Spanish Town and May Pen. Use alternate routes via B12.',                        roles: 'all',                 hours: 24 },
      { message: '🆘 URGENT: National Arena shelter at capacity. Overflow directed to UWI Mona campus. Coordinators please update routing.',   roles: 'coordinator',         hours: 6  },
      { message: '✅ ALL CLEAR: Flood waters receding in Portland. Residents may return home. Exercise caution on roads.',                     roles: 'all',                 hours: 48 },
      { message: '📦 SUPPLY DROP: Water and food packages at Half Way Tree Transport Centre. Bring ID.',                                       roles: 'all',                 hours: 8  },
    ];
    const alertIds = [];
    for (const b of broadcastMessages) {
      const { rows } = await client.query(
      ...
      for (const userId of recipients) {
        ...
        await client.query(
          `INSERT INTO user_alerts (user_id, alert_id, acknowledged, acknowledged_at, delivered_via)
           VALUES ($1, $2, $3, $4, $5)
           ON CONFLICT (user_id, alert_id) DO NOTHING`,
          [userId, alertId, acked, acked ? new Date(Date.now() - Math.floor(Math.random() * 3600000)) : null, pick(['push', 'push', 'in_app'])]
        );
        ackCount++;
      }
    }
    console.log(`   ${alertIds.length} broadcast alerts, ${ackCount} deliveries`);
    ...
    for (const userId of [...volunteerIds, ...responderIds]) {
      const isResponder      = responderIds.includes(userId);
      const tasksCompleted   = isResponder ? 5 + Math.floor(Math.random() * 30)   : Math.floor(Math.random() * 25);
      const hours            = isResponder ? +(20 + Math.random() * 200).toFixed(2) : +(Math.random() * 120).toFixed(2);
      const streakDays       = Math.floor(Math.random() * (isResponder ? 21 : 14));
      const numBadges        = isResponder ? 1 + Math.floor(Math.random() * 5) : Math.floor(Math.random() * 4);
      const badges = [];
      for (let b = 0; b < numBadges; b++) {
        const badge = pick(badgePool);
        if (!badges.find((x) => x.name === badge)) {
      ...
      await client.query(
        `INSERT INTO volunteer_stats (user_id, tasks_completed, hours_contributed, badges, streak_days, last_active_at, leaderboard_opt_in)
         VALUES ($1, $2, $3, $4, $5, NOW() - INTERVAL '${Math.floor(Math.random() * 48)} hours', $6)`,
        [userId, tasksCompleted, hours, JSON.stringify(badges), streakDays, Math.random() > 0.3]
      );
    }
    console.log(`   ${volunteerIds.length + responderIds.length} volunteer stats`);
    ...
    const chatPool = [
      ...
      { from: 'volunteer',   content: 'On my way, ETA 15 minutes.'                                           },
      { from: 'victim',      content: 'Is it safe to return home now?'                                       },
      { from: 'coordinator', content: 'Not yet. We will send an all-clear when it is safe.'                  },
      { from: 'responder',   content: 'Road blocked at Old Hope Road. Taking alternate route via Barbican.'  },
      { from: 'coordinator', content: 'Copy. Updating the route for other responders.'                       },
      { from: 'victim',      content: 'My neighbour is injured and cannot walk. Please bring a stretcher.'   },
    ];
    ...
    let msgCount = 0;
    for (const incId of incidentIds.slice(0, 5)) {
      const threadLen = 3 + Math.floor(Math.random() * 4);
      const thread = [
        pick(chatPool.filter((m) => m.from === 'victim')),
        pick(chatPool.filter((m) => m.from === 'coordinator')),
        pick(chatPool.filter((m) => m.from === 'volunteer')),
      ];
      while (thread.length < threadLen) thread.push(pick(chatPool));
      for (let m = 0; m < thread.length; m++) {
        const tmpl    = thread[m];
        const senderId = pick(senderMap[tmpl.from]);
        const minsAgo  = (thread.length - m) * (5 + Math.floor(Math.random() * 10));
        await client.query(
          `INSERT INTO messages (incident_id, sender_id, content, message_type, delivered, read, created_at)
           VALUES ($1, $2, $3, 'text', $4, $5, NOW() - INTERVAL '${minsAgo} minutes')`,
          [incId, senderId, tmpl.content, Math.random() > 0.1, Math.random() > 0.3]
    ...
    console.log(`   ${msgCount} messages across 5 incident threads`);

    // ── 13. Preparedness guides ─────────────────────────────
    console.log('Seeding preparedness content...');
    const guides = [
      {
        title: 'Hurricane Preparedness Guide', category: 'hurricane', parish: null,
        content: `## Before the Hurricane
- Stock at least 3 days of water (1 gallon per person per day)
- Secure loose outdoor items (zinc sheets, furniture, garbage bins)
- Charge all devices and portable batteries
- Know your nearest shelter — check ODPEM or call 116
- Fill bathtub with water for flushing/cleaning
...
- Listen to JIS for official updates
...
- Boil tap water until water supply is confirmed safe`,
      ...
      {
        title: 'Earthquake Safety', category: 'earthquake', parish: null,
...
- Expect aftershocks
- Do not use elevators
- Text instead of call to keep lines clear for emergencies`,
      },
      {
        title: 'Flood Response Guide', category: 'flood', parish: null,
        content: `## Flood Warning Signs
- Heavy rainfall lasting more than 2 hours
- Rivers and gullies rising rapidly
- Water entering low-lying areas
- Jamaica Met Service flood warnings
...
- Watch for snakes and insects displaced by flooding`,
      ...
      {
        title: 'Emergency Go-Bag Checklist', category: 'general', parish: null,
        content: `## Essential Items (72-Hour Kit)
- Water: 1 gallon per person per day (3-day supply)
- Non-perishable food: canned goods, crackers, dried fruit
- Manual can opener
- Flashlight with extra batteries
...
- Pet food and supplies if needed
- Local map (GPS may not work)`,
      },
      {
        title: 'Fire Safety at Home', category: 'fire', parish: null,
        content: `## Prevention
- Never leave cooking unattended
- Keep flammable items away from stove and heat sources
- Check electrical wiring regularly — frayed wires cause fires
...
- Know 2 exits from every room
- Practice fire drills with your household
- Keep keys near doors
- If trapped, seal door gaps with wet cloth and signal from window`,
      },
      {
        title: 'Kingston & St Andrew Shelters', category: 'shelters', parish: 'Kingston',
        content: `## Official Emergency Shelters
- National Arena, Independence Park — capacity 3,000
- National Indoor Sports Centre — capacity 2,000
- Excelsior High School — capacity 500
- Wolmer's Boys' School — capacity 400
...
Contact ODPEM: 876-906-9674 or dial 116`,
      ...
      {
        title: 'St James & Montego Bay Shelters', category: 'shelters', parish: 'Saint James',
        content: `## Official Emergency Shelters
- Montego Bay Convention Centre — capacity 2,500
- Catherine Hall Primary — capacity 350
- St James High School — capacity 400
...
Contact ODPEM Western: 876-952-1838`,
      ...
      {
        title: 'Portland Parish Flood Guide', category: 'flood', parish: 'Portland',
        content: `## Flood-Prone Areas
- Port Antonio town centre (near Rio Grande)
- Hope Bay river basin
- Buff Bay lowlands
- Long Bay coastal road
...
- Evacuate from river banks when heavy rain persists
- Use John Crow Mountains high roads as escape routes
- Shelter at Titchfield High or Portland Parish Church
...
Portland is one of Jamaica's wettest parishes — always have a go-bag ready during hurricane season (June–November).`,
    ...
    console.log('Seeding media records...');
    const mediaTemplates = [
      'flood_damage_01.jpg', 'collapsed_roof_01.jpg', 'road_blocked_01.jpg',
      'rescue_team_01.jpg',  'shelter_setup_01.jpg',  'supply_drop_01.jpg',
      'fire_scene_01.jpg',   'medical_aid_01.jpg',
    ];
    let mediaCount = 0;
    for (const incId of incidentIds.slice(0, 10)) {
      const numPhotos = 1 + Math.floor(Math.random() * 3);
      for (let m = 0; m < numPhotos; m++) {
        const filename = pick(mediaTemplates);
        const p        = pick(parishes);
        const pt       = randomPointNear(p);
        const hoursAgo = Math.floor(Math.random() * 48);
        await client.query(
          `INSERT INTO media (incident_id, uploaded_by, storage_key, content_type, lat, lon, status, uploaded_at, created_at)
           VALUES ($1, $2, $3, 'image/jpeg', $4, $5, 'uploaded',
                   NOW() - INTERVAL '${hoursAgo} hours',
                   NOW() - INTERVAL '${hoursAgo} hours')`,
          [incId, pick([...victimIds, ...responderIds]), `incidents/${incId}/${filename}`, pt.lat, pt.lon]
        );
        mediaCount++;
      }
    }
    ...
    // ── 15. SMS/WhatsApp incidents ──────────────────────────
    console.log('Seeding SMS/WhatsApp incidents...');
    const smsMessages = [
      { raw: 'HELP MEDICAL 14 Palm Street Kingston 2 people',  type: 'medical_emergency', location: '14 Palm Street Kingston', people: 2 },
      { raw: 'TRAPPED old warehouse by the harbour Port Royal', type: 'trapped',           location: 'Port Royal harbour',      people: 1 },
      { raw: 'SUPPLIES need water and food St Andrew 5 people', type: 'need_supplies',     location: 'Saint Andrew',            people: 5 },
      { raw: 'SHELTER 8 people need shelter in Portmore',       type: 'shelter',           location: 'Portmore',                people: 8 },
      { raw: 'MEDICAL elderly man collapsed at Half Way Tree',  type: 'medical_emergency', location: 'Half Way Tree',           people: 1 },
    ];
    for (const sms of smsMessages) {
      await client.query(
        `INSERT INTO sms_incidents (from_number, raw_message, parsed_type, parsed_location, parsed_people, incident_id, status)
         VALUES ($1, $2, $3, $4, $5, $6, 'created')`,
        ['+1876' + Math.floor(1000000 + Math.random() * 9000000), sms.raw, sms.type, sms.location, sms.people, pick(incidentIds)]
      );
    }
    console.log(`   ${smsMessages.length} SMS/WhatsApp incidents`);
    ...
    console.log(`  Coordinators:         ${coordinatorIds.length}`);
    console.log(`  Incidents:            ${incidentIds.length}`);
    console.log(`  Org resources:        ${resourceIds.length}`);
    console.log(`  Allocations:          10`);
    console.log(`  Notifications:        ${notifCount}`);
    console.log(`  Broadcast alerts:     ${alertIds.length}`);
    console.log(`  Volunteer stats:      ${volunteerIds.length + responderIds.length}`);
    console.log(`  Messages:             ${msgCount}`);
    console.log(`  Preparedness guides:  ${guides.length}`);
    console.log(`  Media records:        ${mediaCount}`);
    console.log(`  SMS/WA incidents:     ${smsMessages.length}`);

    console.log('\nLogin credentials:');
    console.log('\n  -- Volunteers --');
    for (const v of volunteers) console.log(`  ${v.username} / ${v.password}`);
    console.log('\n  -- Responders --');
    for (const r of responders) console.log(`  ${r.username} / ${r.password}`);
    console.log('\n  -- Coordinators --');
    for (const c of coordinators) console.log(`  ${c.username} / ${c.password}`);
    console.log('\n  Victims: any registered phone + any 6-digit OTP (TESTING_MODE=true)\n');
