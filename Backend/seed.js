...
function randomPointInKingston() {
  return {
    lat: 17.94 + Math.random() * 0.12,
    lon: -76.84 + Math.random() * 0.12,
  };
}

function assignVolunteerResources(items, minCount, maxCount) {
  const chosen = pickSome(items, minCount, maxCount);
  return chosen.map(item => ({
    item,
    quantity: 1 + Math.floor(Math.random() * 15),
    status: Math.random() > 0.25 ? 'available' : 'deployed',
  }));
}
  ...
  try {
    ...
    for (const parish of parishes) {
      ...
      for (let i = 0; i < numVolunteers; i++) {
        ...
        volunteerIndex++;
        const pt = randomPointInKingston();
        const days = Math.floor(Math.random() * 60);
        const resources = assignVolunteerResources(resourceItemList, 3, 8);
        const username = `${v.username}_${volunteerIndex}`;
        const { rows } = await client.query(
