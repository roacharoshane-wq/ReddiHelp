...
function normalizeResourcesList(resources) {
  if (!resources) return [];
...
function getResourceMatchKeywords(incidentType) {
  return RESOURCE_MATCH_MAP[normalizeResourceName(incidentType)] || RESOURCE_MATCH_MAP.other;
}

function scoreResourceMatch(incidentType, resources) {
  const keywords = getResourceMatchKeywords(incidentType).map(normalizeResourceName);
  const normalizedResources = normalizeResourcesList(resources);

  const matches = normalizedResources.flatMap((resource) => {
    const resourceName = normalizeResourceName(
      resource.type || resource.name || resource.category || resource.resource || resource.label
    );
    const resourceTags = Array.isArray(resource.tags)
      ? resource.tags.map(normalizeResourceName)
      : [];

    const matchedKeywords = keywords.filter(
      (keyword) => resourceName.includes(keyword) || resourceTags.includes(keyword)
    );

    if (matchedKeywords.length === 0) return [];

    const quantity = Math.max(1, Math.min(15, Number(resource.quantity) || 1));
    const status = String(resource.status || 'available').toLowerCase();
    const statusFactor = status === 'deployed' ? 0.75 : 1;
    const quantityFactor = quantity / 15;

    return [{
      type: resource.type || resource.name || resource.category || 'resource',
      quantity,
      status,
      score: Math.min(1, quantityFactor * statusFactor + 0.25),
      matchedKeywords,
    }];
  });
  ...
  return {
    score: Math.min(1, total / Math.max(1, Math.min(3, keywords.length))),
    matches: matches.sort((a, b) => b.score - a.score),
  };
}

function scoreIncidentCandidate({ incidentType, distance, maxDistance, resources }) {
  const proximityScore = 1 - Math.min(distance, maxDistance) / maxDistance;
  const resourceMatch = scoreResourceMatch(incidentType, resources);

  return {
    proximityScore,
    resourceMatchScore: resourceMatch.score,
    totalScore: (proximityScore * 0.7) + (resourceMatch.score * 0.3),
    resourceMatches: resourceMatch.matches,
  };
}
  ...
  try {
    ...
    if (candidates.rows.length === 0) {
      const fallback = await pgPool.query(
        `SELECT u.id, u.phone, u.role, u.skills, u.resources, u.vehicle, u.availability,
                u.active_location_name, u.last_lat, u.last_lon,
                ST_Distance(u.location, ST_GeogFromText($1)) as distance
         FROM users u
         WHERE u.role IN ('volunteer', 'responder')
           AND u.availability = 'available'
      ...
      candidates.rows = fallback.rows;
    }

    const scored = candidates.rows.map(c => {
      const candidateParish = normalizeParishName(c.active_location_name);
      const scores = scoreIncidentCandidate({
      ...
      return {
        ...
        activeLocationName: c.active_location_name,
        distance: Math.round(c.distance),
        parishMatch: incidentParish && candidateParish && incidentParish === candidateParish,
        proximityScore: scores.proximityScore,
        resourceMatchScore: scores.resourceMatchScore,
        resourceMatches: scores.resourceMatches,
  ...
  try {
    const result = await pgPool.query(
      `SELECT u.id, u.username, u.phone, u.role, u.availability,
              ...
              u.last_lat, u.last_lon, u.last_location_at,
              CASE
                WHEN u.availability = 'available' THEN 'contactable'
                WHEN u.availability = 'on_task' THEN 'busy'
                WHEN u.availability = 'unavailable' THEN 'offline'
                ELSE 'contactable'
  ...
  try {
    ...
    const scored = candidates.rows.map(c => {
      ...
      const confidence = Math.min(
        100,
        Math.round(
          ((scores.proximityScore * 0.7) + (scores.resourceMatchScore * 0.3)) * 100
        )
      );
      ...
      if (exp > 0) reasoning.push(`Completed ${exp} similar tasks`);
      if (c.vehicle) reasoning.push(`Has ${c.vehicle}`);
      if (scores.resourceMatches.length > 0) {
        reasoning.push(`Resources: ${scores.resourceMatches.map((r) => r.type).join(', ')}`);
      }
      if (incidentParish && candidateParish && incidentParish === candidateParish) reasoning.push('Preferred parish match');
...
app.post('/api/auto-dispatch/run', authenticateToken, authorize('coordinator', 'admin'), async (req, res) => {
  try {
    ...
    for (const inc of unassigned.rows) {
      ...
      let fallbackUsed = false;
      if (candidates.rows.length === 0) {
        // Level-1 fallback: nearest available volunteer WITH a GPS location (no distance cap)
        const fallback = await pgPool.query(
          `SELECT u.id, u.username, u.skills, u.availability,
                  COALESCE(ST_Distance(u.location, ST_GeogFromText($1)), 0) as distance
           FROM users u
           WHERE u.role IN ('volunteer','responder')
             AND u.availability = 'available'
             AND u.last_lat IS NOT NULL
        ...
        if (fallback.rows.length > 0) {
          ...
          // Level-2 fallback: any available volunteer, even without GPS data
          const anyVolunteer = await pgPool.query(
            `SELECT u.id, u.username, u.skills, u.availability, 0 as distance
             FROM users u
             WHERE u.role IN ('volunteer','responder')
               AND u.availability = 'available'
          ...
          if (anyVolunteer.rows.length === 0) {
            manualReview.push({ incidentId: inc.id, reason: 'No available volunteers in the system' });
            continue;
          }

          candidates.rows = anyVolunteer.rows;
          fallbackUsed = true;
        }
      }

      const best = candidates.rows[0];
      const score = scoreIncidentCandidate({
        incidentType: inc.type,
      ...
      if (fallbackUsed) {
        ...
        await pgPool.query("UPDATE users SET availability = 'on_task' WHERE id = $1", [best.id]);
        await pgPool.query('INSERT INTO incident_history (incident_id, from_status, to_status, changed_by) VALUES ($1, $2, $3, $4)', [inc.id, 'active', 'in-progress', req.user.id]);
        await pgPool.query('INSERT INTO dispatch_decisions (incident_id, volunteer_id, confidence, decision) VALUES ($1, $2, $3, $4)', [inc.id, best.id, confidence, 'auto_assigned_fallback']);
        io.to('coordinators').emit('incident:updated', { id: inc.id, status: 'in-progress', assignedTo: best.id });
        autoAssigned.push({ ...entry, reason: 'Nearest available fallback' });
