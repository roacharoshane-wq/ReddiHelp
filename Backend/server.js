...
app.get('/api/incidents/:id/match-enhanced', authenticateToken, authorize('coordinator', 'admin'), async (req, res) => {
  ...
  try {
    ...
    const relevantResources = resourceMap[inc.type] || resourceMap.other;
    ...
    const scored = candidates.rows.map(c => {
      ...
      const confidence = Math.min(
        100,
        Math.round(
          (distScore * 0.7 + resourceMatchScore * 0.3 + activeLocBoost + parishBoost) * 100
        )
      );
      ...
      if (matchedResources.length > 0) reasoning.push(`Resources: ${matchedResources.join(', ')}`);
...
app.post('/api/auto-dispatch/run', authenticateToken, authorize('coordinator', 'admin'), async (req, res) => {
  try {
    ...
    for (const inc of unassigned.rows) {
      ...
      let candidates = await pgPool.query(
        `SELECT u.id, u.username, u.skills, u.resources, u.availability,
                ST_Distance(u.location, ST_GeogFromText($1)) as distance
         FROM users u
      ...
      if (candidates.rows.length === 0) {
        const fallback = await pgPool.query(
          `SELECT u.id, u.username, u.skills, u.resources, u.availability,
           ...
           WHERE u.role IN ('volunteer','responder') AND u.availability = 'available'
             AND u.last_lat IS NOT NULL
           ORDER BY ST_Distance(u.location, ST_GeogFromText($1)) ASC LIMIT 1`,
          [`POINT(${inc.lon} ${inc.lat})`]
        );
      ...
      const scoredCandidates = candidates.rows.map(c => {
        const distScore = 1 - Math.min(c.distance, 20000) / 20000;
        const userResources = Array.isArray(c.resources) ? c.resources.map(r => String(r.item).toLowerCase()) : [];
        const matchedResources = relevantResources.filter(r => userResources.includes(r.toLowerCase()));
        const resourceMatchScore = relevantResources.length > 0 ? Math.min(matchedResources.length / relevantResources.length, 1) : 0;
        return {
          ...c,
          totalScore: distScore * 0.7 + resourceMatchScore * 0.3,
          resourceMatchScore,
          matchedResources,
        };
      });
      ...
      const confidence = Math.round(best.totalScore * 100);
